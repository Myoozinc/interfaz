...
export class AIGenerator {
  ...
  async generateAppCode(
    ...
    const systemPrompt = `Eres NONA AI, un ingeniero senior y diseñador de software y videojuegos de clase mundial.
Tu objetivo es crear o modificar aplicaciones web completas, interactivas, funcionales y de altísimo nivel estético.
REGLAS OBLIGATORIAS:
1. Devuelve TODO el código HTML/CSS/JS autocontenido en un ÚNICO bloque de código \`\`\`html con <!DOCTYPE html>.
2. Usa Tailwind CSS (CDN https://cdn.tailwindcss.com), Lucide icons (https://unpkg.com/lucide@latest), y Three.js si se solicitan juegos o 3D.
3. Si el usuario pide modificar o agregar algo a un código existente, PRESERVA todo el código funcional previo e integra los cambios solicitados.
4. Escribe código 100% real, interactivo y ejecutable directamente en el navegador.`;
...
Genera el código HTML completo actualizado con los cambios solicitados aplicados sobre el código existente. Devuélvelo en un único bloque \`\`\`html.`
      : `Desarrolla la siguiente aplicación web completa, interactiva y con diseño moderno: "${prompt}". Devuelve todo el código en un único bloque \`\`\`html completo listo para renderizar.`;
    ...
    for (const endpoint of endpointsToTry) {
      try {
        ...
        onToken('⏳ Conectando con tu modelo ' + this.selectedModel + ' en tu Mac...', '', false);
